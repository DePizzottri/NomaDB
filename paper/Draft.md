...
**Теорема (_Шпильрайн_)**:
    Для любого посета существет линейное расширение [ссылка].
...
**Теорема (_Дилворт_)**:
    Минимальное количество цепей, на которое можно разбить посет равно количеству элементов в максимально антицепи [ссылка].
...
**Теорема (_Хирагучи_)**:
    Размерность посета P не превосходит его ширины [ссылка].
...
**Утверждение (_Янакаккис_)** Задача определения того, что посет имеет размерность больше 2х является NP-полной [ссылка].
...
**Теорема (Троттер)**:
    Если граф G<sub>c</sub>(P) это граф несовместных критических пар посета P, который не является линейным порядком. Тогда размерность посета P будет равна 2 если, и только если χ(G) == 2 [ссылка].
