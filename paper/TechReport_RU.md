...
Кроме того, возникают дополнительные требования в виде массовой репликации - для достижения распределения нагрузки, децентрализации и бóльшему повышению доступности и гео-распределенности - системы хранения данных становятся глобальными.
...
* Hash Histories[[7]](http://oceanstore.cs.berkeley.edu/publications/papers/pdf/hh_icdcs03_kang.pdf). Полностью хранит посет исполнения (см. ниже) в виде графа с узлами, содержащими хэш от данных. С одной стороны содержит избыточную информацию, с другой - имеет возможность обнаружения одинаковых копий, оказавшихся таковыми случайно, и чего не следует из причинностных связей.
...
Основываясь на опыте распределенных баз данных с master-master репликацией и обобщая способы согласования данных в системах с нестрогим согласованием данных были выведены Безконфликтные Реплицируемые Структуры Данных (англ. CRDT - Conflict Free Replicated Data Types). Использование таких структур гарантируют _строгую_ согласованность в конечном итоге [3]. Кроме прочего, CRDT в явном виде уже использует, или собираются вводить множество крупных собременных СУБД: например **MS[ссылка], Redis[ссылка], [ссылка с тех слайдов]**

Существуют два типа CRDT [[23]](https://hal.inria.fr/inria-00555588/document): 
* CmRDT - ориентированный на операции (коммутативные, op-based CRDT,  Commmutative RDT). В которых репликация осуществляется атомарным распространением операций модификации. Такие операции должны быть коммутативными для того, чтобы структура данных сходилась к единому стостоянию.
* СvRDT - ориентированные на состояния (конвергентные (сходящиеся), state-based CRDT, Convergent RDT). В которых репликация осуществляется распространением состояния, которые в свою очередь сливаются воедино при помощи операции merge. Существует модификация δ-CRDT, в которых репликация производится слиянием не полного состояния, а лишь измененной части.

В свою очередь каждая из разновидностей отличается приемуществами и недостатками:
+ CmRDT
...
Множество P, c заданным на нем бинарном отношение, обладающее свойствами рефлексивности и транзитивности называется **предпорядком**.
...
**Размерность посета** (P, →) - это реалайзер минимального размера.
...
**тут связь размерности и размера часов**
