...
##### Ψ(3, S)
Три процесса полностью синхронизируются при помощи 3х синхронизаций и образуют ровно две различных фигуры, изображенных на рисунке [рисунок](). Первая фигура является случаем полской сетки, описанной ниже. Кроме машинной проверки оба случая были проверенны вручную построением риалайзера. Композиция этих фигур также всегда будет размерности 2. Обе фигуры можно отнести к регулярным и общим.
...
* Описание частно го случая модели синхронизирующихся процессов, вместе с выделенными сущбностями Ψ и mΨ.
* Описание алгоритма проверки посета на размерность равную двум. Алгоритм является модульным и при замене этапо на более общие может служить алгоритмом нахождения размерности любого посета.
...
* Демонстрация того факта, что три процесса при синхронизациях всегда образуют постер размерности 2.
